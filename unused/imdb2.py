...
def main(title, opts):
  ...
  print("https://www.themoviedb.org/movie/%s" % data.get('tmdb', {}).get('id'))
  if data['poster']:
    print("Poster:")
    print("https://image.tmdb.org/t/p/w1280/%s" % data['poster'])
  print("Searches:")
...
def print_to(stream, data, encode=True):
  ...
  if data['rotten_url']:
    if data['rotten_url'].startswith('http'):
      ...
      stream.write("Rotten url: <http:%s>   \n" % data['rotten_url'])
  stream.write("Audience  : %s %s   \n" % (data['audience_score'], data['audience_rating']))
  stream.write("Critics   : %s %s   \n" % (data['critics_score'], data['critics_rating']))
  stream.write("\n")
  if data['tmdb']:
    stream.write("TMDB link: <https://www.themoviedb.org/movie/%s>   \n" % data['tmdb']['id'])
    # stream.write("TMDB rate: %d(%d)   \n" % (data['tmdb']['popularity'], data['tmdb']['vote_count']))
    stream.write("\n")
  stream.write("* * * * * * * * * * *")
  stream.write("\n\n")
